...
class TriviaTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.database_path = 'postgresql://postgres:fmmqsb100@localhost:5432/trivia_test'
    ...
    def test_CreateQuestionSuccessfully(self):
        res = self.client().post('/questions', json={'question':'Who is the player with the most world cup goals till 2014?',
                                                     'answer':'Ronaldo','category':6,'difficulty':3})
        questions = Question.query.filter(Question.answer == 'Ronaldo').one_or_none()

        self.assertEqual(res.status_code, 200)
        self.assertEqual(questions.answer, 'Ronaldo')

    def test_CreateQuestion422Error(self):
        res = self.client().post('/questions', json={'question':'Who is the player with the most world cup goals till 2014?',
                                                     'answer':'Ronaldo','category':6})
